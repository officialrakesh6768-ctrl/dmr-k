...
import { ArrowRight, Building2, Fence, Shovel, Trees } from "lucide-react";
...
const previews = [
  {icon:Shovel, n:"01", title:"Excavating", copy:"Precision site preparation, trenching, foundation work and finish grading."},
  {icon:Building2, n:"02", title:"Demolition", copy:"Controlled removal, careful clearing and responsible site cleanup."},
  {icon:Trees, n:"03", title:"Landscaping", copy:"Hard-working outdoor spaces shaped with a refined, lasting finish."},
  {icon:Fence, n:"04", title:"Fencing", copy:"Purpose-built boundaries installed cleanly across residential and rural sites."},
];
...
function HomePage() { return <>
  <section className="relative flex min-h-[92svh] items-end overflow-hidden pt-20">
    ...
    <div className="relative mx-auto w-full max-w-7xl px-5 pb-16 lg:px-8 lg:pb-20"><div className="max-w-4xl animate-rise">
      ...
      <div className="mt-9 flex flex-wrap gap-3"><Button asChild variant="industrial" size="xl"><Link to="/enquiry">Get a free estimate <ArrowRight/></Link></Button><Button asChild variant="industrialOutline" size="xl"><Link to="/services">Explore services</Link></Button></div>
    </div></div>
  </section>
  <section className="border-y border-border bg-surface"><div className="mx-auto grid max-w-7xl grid-cols-2 lg:grid-cols-4">{[["10+","Years built tough"],["9","Core services"],["100%","Project focus"],["1","Trusted crew"]].map(([v,l])=><div key={l} className="border-border p-6 odd:border-r lg:border-r lg:p-8"><p className="font-display text-5xl font-black text-primary">{v}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">{l}</p></div>)}</div></section>
